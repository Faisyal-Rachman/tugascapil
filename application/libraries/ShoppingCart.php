...
class ShoppingCart
{
    ...
    public function getCartItems()
    {
        ...
        foreach ($result['array'] as &$article) {
            ...
            $article['sum_price'] = $article['sum_price'];
            $article['price'] = $article['price'] != '' ? $article['price'] : 0;
            $article['sum_point'] = floatval($article['point']) * $count_articles[$article['id']];
            $finalPoint = $finalPoint + $article['sum_point'];
            
        }
        $result['finalSum'] = $finalSum;
